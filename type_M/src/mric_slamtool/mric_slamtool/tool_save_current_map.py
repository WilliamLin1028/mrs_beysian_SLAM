...
class SaveRobotMap_InServer(Node):
    ...
    def __init__(self):
        ...
        self.map_info_file_path = os.path.join(bringup_dir, 'database', 'tool_map_info.txt')
        self.map_data_file_path = os.path.join(bringup_dir, 'database', 'tool_map_data.txt')
        self.map_msg = None
...
def main():
    ...
    future = agent.node_action()
    rclpy.spin_until_future_complete(agent, future)
    # rclpy.spin(agent)
    agent.destroy_node()
    rclpy.shutdown()
